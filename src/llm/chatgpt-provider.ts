...
if (!process.env.OPENAI_API_KEY) {
	console.warn("OPENAI_API_KEY environment variable is not set. OpenAI LLM calls will fail.");
	// Potentially throw an error here if strict checking is required at module load time
	// throw new Error("OPENAI_API_KEY environment variable is not set.");
}

const DEFAULT_CHATGPT_MODEL_NAME = "o3";
...
export class ChatGptProvider implements LlmProvider {
	...
	async generate(prompt: string, options?: LlmOptions): Promise<string> {
		...
		try {
			console.log(
				`ChatGptProvider: Calling LLM (Model: ${modelName}) with prompt (hash: ${promptHash.substring(
					0,
					10
				)}...)...`
			);
