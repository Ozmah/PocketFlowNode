...
if (!process.env.ANTHROPIC_API_KEY) {
  console.warn(
    "ANTHROPIC_API_KEY environment variable is not set. Anthropic LLM calls will fail."
  );
  // Potentially throw an error here if strict checking is required at module load time
  // throw new Error("ANTHROPIC_API_KEY environment variable is not set.");
}

const DEFAULT_CLAUDE_MODEL_NAME = "claude-instant-1.2";
...
export class ClaudeProvider implements LlmProvider {
  ...
  async generate(prompt: string, options?: LlmOptions): Promise<string> {
    ...
    try {
      console.log(`ClaudeProvider: Calling LLM (Model: ${modelName}) with prompt (hash: ${promptHash.substring(0,10)}...)...`);
