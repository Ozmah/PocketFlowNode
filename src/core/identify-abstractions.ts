...
export async function identifyAbstractions(
	...
	const { language = "english", useCache = true, maxAbstractions = 10 } = options;
...
${
	language !== "english"
		? `IMPORTANT: Please provide the 'name' and 'description' fields in ${language}. The file paths and indices should remain as they are.\nExample for ${language}:\nname: (name in ${language})\ndescription: (description in ${language})\nfile_indices: [0, 1]`
		: ""
}
...
`;
	...
	try {
		...
		if (!yamlMatch || !yamlMatch[1]) {
			...
			try {
				...
				throw new Error(
					`Failed to parse LLM response as YAML (no explicit block and direct parse failed): ${
						e.message
		...
		throw new Error(
			`Invalid YAML from LLM: ${e.message}\nRaw YAML part:\n${
	...
	if (!Array.isArray(rawAbstractions)) {
		...
		if (
			rawAbstractions &&
			typeof rawAbstractions === "object" &&
			rawAbstractions.name &&
			rawAbstractions.description &&
			rawAbstractions.file_indices
		) {
			console.warn("LLM returned a single object, not an array. Wrapping it.");
			rawAbstractions = [rawAbstractions];
		} else {
			throw new Error(
				`LLM output is not an array of abstractions. Received: ${typeof rawAbstractions}\nContent:\n${JSON.stringify(
					rawAbstractions,
					null,
					2
				).substring(0, 500)}...`
	...
	for (const rawAbs of rawAbstractions) {
		...
		if (!Array.isArray(rawAbs.file_indices) || rawAbs.file_indices.length === 0) {
			console.warn(
				`Skipping abstraction "${rawAbs.name}" with missing, empty, or non-array file_indices:`,
				rawAbs.file_indices
			);
			continue;
		}
		...
		try {
			...
			console.warn(
				`Skipping abstraction "${rawAbs.name}" due to invalid file_indices: ${e.message}`,
				rawAbs.file_indices
			);
		}
	}
