...
const DEFAULT_MODEL_NAME = "gemini-pro"; // A common default model
...
export async function callLlm(
  ...
  try {
    ...
    const model = genAI.getGenerativeModel({ 
        ...
        // Recommended safety settings for code generation or technical content
        safetySettings: [
            {
    ...
    console.log(`Calling LLM (Model: ${GEMINI_MODEL_NAME}) with prompt (hash: ${promptHash.substring(0,10)}...)...`);
    ...
    } else if (error.details) { // gRPC-like error structure (used by Google AI SDK)
